...
ORIGIN_SUGGESTION_MIN_QUERY_LENGTH = 3
ORIGIN_SUGGESTION_MAX_QUERY_LENGTH = 100
ORIGIN_SUGGESTION_RESULT_LIMIT = 5
ORIGIN_SUGGESTION_UPSTREAM_RESULT_LIMIT = 15
ORIGIN_SUGGESTION_TIMEOUT_SECONDS = 4
...
class OriginSuggestionQueryError(ValueError):
    """Raised when an origin-suggestion query exceeds public bounds."""


class OriginSuggestionServiceError(TransitousError):
    """Raised when Transitous cannot provide usable origin suggestions."""


class OriginSuggestionTimeoutError(OriginSuggestionServiceError):
    """Raised when Transitous origin geocoding exceeds the short timeout."""
...
def find_origin_suggestions(query: str) -> list[dict]:
    """Return a bounded Nightways-owned projection of Transitous places."""

    if not isinstance(query, str):
        raise OriginSuggestionQueryError(
            "Origin suggestion query must be text."
        )

    normalized_query = " ".join(query.split())
    if len(normalized_query) < ORIGIN_SUGGESTION_MIN_QUERY_LENGTH:
        return []
    if len(normalized_query) > ORIGIN_SUGGESTION_MAX_QUERY_LENGTH:
        raise OriginSuggestionQueryError(
            "Origin suggestion query is too long."
        )

    matches = _request_origin_suggestion_matches(normalized_query)
    candidates = _origin_suggestion_candidates(matches)
    return _project_origin_suggestion_candidates(candidates)[
        :ORIGIN_SUGGESTION_RESULT_LIMIT
    ]


def _request_origin_suggestion_matches(query_text: str) -> list[dict]:
    query = urlencode(
        {
            "text": query_text,
            "type": "PLACE",
            "language": "en",
            "numResults": ORIGIN_SUGGESTION_UPSTREAM_RESULT_LIMIT,
        }
    )
    request = Request(
    ...
    try:
        with urlopen(
            request,
            timeout=ORIGIN_SUGGESTION_TIMEOUT_SECONDS,
        ) as response:
            matches = json.load(response)
    except HTTPError as error:
        raise OriginSuggestionServiceError(
            "Transitous origin suggestions returned "
            f"HTTP {error.code}. Please try again later."
        ) from error
    except TimeoutError as error:
        raise OriginSuggestionTimeoutError(
        ...
        raise OriginSuggestionServiceError(
            "Could not reach Transitous origin suggestions. "
            "Please try again later."
        ) from error
    except (OSError, UnicodeError, json.JSONDecodeError) as error:
        raise OriginSuggestionServiceError(
            "Transitous origin suggestions returned an unreadable response."
        ) from error
...
def _origin_suggestion_candidates(matches: list[dict]) -> list[dict]:
    candidates = []
    seen_provider_ids = set()

    for match in matches:
        candidate = _origin_candidate_from_match(match)
        if candidate is None or candidate.country_code is None:
            continue
        ...
        country, country_qualifier = _origin_suggestion_country(
            match,
            candidate.country_code,
        )
        area_entries = _origin_suggestion_area_entries(
            match,
            candidate.name,
            country,
        )
        region = area_entries[0][1] if area_entries else None
        candidates.append(
            {
                "name": candidate.name,
                "country": country,
                "country_code": candidate.country_code,
                "country_qualifier": country_qualifier,
                "region": region,
                "areas": area_entries,
            }
        )
...
def _project_origin_suggestion_candidates(candidates: list[dict]) -> list[dict]:
    groups = defaultdict(list)
    for candidate in candidates:
        key = (
            _normalized_city_name(candidate["name"]),
            candidate["country_code"],
        )
    ...
    for candidate in candidates:
        ...
        if len(group) > 1:
            qualifier = _distinct_origin_suggestion_qualifier(
                candidate,
                group,
            )
            if qualifier is not None:
                candidate["region"] = qualifier

        if qualifier is None and _safe_origin_query_qualifier(
            candidate["country_qualifier"]
        ):
            qualifier = candidate["country_qualifier"]
        ...
        label_context = [
            context
            for context in (candidate["region"], candidate["country"])
            if isinstance(context, str) and context.strip()
        ]
        label = candidate["name"]
        if label_context:
            label = f"{label} — {', '.join(label_context)}"
...
def _origin_suggestion_country(
    match: dict,
    country_code: str,
) -> tuple[str, str | None]:
    for area in match.get("areas") or []:
        if not isinstance(area, dict) or area.get("adminLevel") != 2:
            continue
        area_name = _normalized_suggestion_area_name(area.get("name"))
        if area_name is not None:
            return area_name, area_name
    return country_code, None


def _origin_suggestion_area_entries(
    match: dict,
    origin_name: str,
    country: str,
) -> tuple[tuple[float, str], ...]:
    origin_key = _normalized_city_name(origin_name)
    country_key = _normalized_city_name(country)
    entries = []
    seen_names = set()
    ...
    for area in match.get("areas") or []:
        if not isinstance(area, dict):
            continue
        admin_level = area.get("adminLevel")
        if (
            isinstance(admin_level, bool)
            or not isinstance(admin_level, (int, float))
            or not math.isfinite(admin_level)
            or not 2 < admin_level <= 8
        ):
            continue

        area_name = _normalized_suggestion_area_name(area.get("name"))
        if area_name is None or not _safe_origin_query_qualifier(area_name):
            continue
        area_key = _normalized_city_name(area_name)
        if area_key in {origin_key, country_key} or area_key in seen_names:
            continue
        seen_names.add(area_key)
        entries.append((float(admin_level), area_name))

    entries.sort(key=lambda entry: entry[0])
    return tuple(entries)


def _distinct_origin_suggestion_qualifier(
    candidate: dict,
    group: list[dict],
) -> str | None:
    area_counts = defaultdict(int)
    for group_candidate in group:
        for _, area_name in group_candidate["areas"]:
            area_counts[_normalized_city_name(area_name)] += 1

    for _, area_name in candidate["areas"]:
        if area_counts[_normalized_city_name(area_name)] == 1:
            return area_name
    return None


def _normalized_suggestion_area_name(value) -> str | None:
...
def _safe_origin_query_qualifier(value: str | None) -> bool:
